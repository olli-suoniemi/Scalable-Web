<script>
  ...
  const fetchQuestionDetailsInitial = async () => {
    ...
    try {
      const response = await fetch(`/api/question/${questionId}?page=${1}&limit=${limit}`);
  ...
  // Function to fetch the question details and its answers when reached bottom of the page
  const fetchQuestionDetails = async (page = 1) => {
    try {
      const response = await fetch(`/api/question/${questionId}?page=${page}&limit=${limit}`);
      if (!response.ok) throw new Error('Question not found'); 
  ...
  const upvoteAnswer = async (answerId) => {
    try {
      const response = await fetch(`/api/answer/upvote/${answerId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      <ul class="mt-2">
        {#each answers as answer}
          <li class="mb-4 border-b pb-2">
            ...
            <p class="text-gray-500">Upvotes: {answer.upvotes || 0}</p>
